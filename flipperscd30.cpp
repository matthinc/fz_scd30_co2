...
bool FlipperSCD30::send_command(std::vector<uint8_t> cmd) {
    WITH_I2C(furi_hal_i2c_tx(&furi_hal_i2c_handle_external, (0x61 << 1), cmd.data(), cmd.size(), 50));
}

bool FlipperSCD30::send_command_and_read(std::vector<uint8_t> cmd, uint8_t* result, int len) {
    WITH_I2C(furi_hal_i2c_trx(&furi_hal_i2c_handle_external, (0x61 << 1), cmd.data(), cmd.size(), result, len, 50));
}
...
SCD30Data FlipperSCD30::read_measurements() {
    ...
    if (result.result_valid) {
        ...
        result.ts = std::to_string(datetime.year) + "-" + std::to_string(datetime.month) + "-" + std::to_string(datetime.day) + " " +
...
static int32_t run(void* context) {
    FlipperSCD30WorkerThread* worker_context = reinterpret_cast<FlipperSCD30WorkerThread*>(context);
...
FlipperSCD30WorkerThread::FlipperSCD30WorkerThread(int interval) : interval(interval) {
